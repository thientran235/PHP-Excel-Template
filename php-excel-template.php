<?php
/**
 * Library for generating reports for PC Builder Project
 *
 * This class defines all code necessary to run during the plugin's activation.
 *
 * @since      1.0.0
 * @package    WCVN_PC_Builder
 * @subpackage WCVN_PC_Builder/includes
...
class PHP_Excel_Template {
	...
	//styling
	private $_headerStyleArray = array( 'font' => array( 'bold' => true, 'color' => array( 'rgb' => 'FFFFFF' ) ), 'fill' => array( 'type' => \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID, 'startcolor' => array( 'rgb' => '4E5A7A' ) ) );
	private $_footerStyleArray = array( 'font' => array( 'bold' => true ), 'fill' => array( 'type' => \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID, 'startcolor' => array( 'rgb' => 'E4E8F3' ) ) );
	private $_headerGroupStyleArray = array( 'alignment' => array( 'horizontal' => \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_LEFT ), 'font' => array( 'bold' => true ), 'fill' => array( 'type' => \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID, 'startcolor' => array( 'rgb' => '8DB4E3' ) ) );
	private $_footerGroupStyleArray = array( 'font' => array( 'bold' => true ), 'fill' => array( 'type' => \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID, 'startcolor' => array( 'rgb' => 'C5D9F1' ) ) );
	private $_noResultStyleArray = array( 'borders' => array( 'outline' => array( 'style' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN ) ), 'alignment' => array( 'horizontal' => \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER ), 'font' => array( 'bold' => true ), 'fill' => array( 'type' => \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID, 'startcolor' => array( 'rgb' => 'FFEBA5' ) ) );
	private $_headingStyleArray = array( 'font' => array( 'bold' => true, 'color' => array( 'rgb' => '4E5A7A' ), 'size' => '24' ), 'alignment' => array( 'horizontal' => \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER ) );
	//\PhpOffice\PhpSpreadsheet\Spreadsheet objects
	private $objReader;
	private $objPHPExcel;
	...
	public function loadTemplate( $template = '' ) {
		...
		$inputFileType        = \PhpOffice\PhpSpreadsheet\IOFactory::identify( $this->_templateDir . $this->_template );
		//TODO: better control of allowed input types
		//load template file into \PhpOffice\PhpSpreadsheet\Spreadsheet objects
		$this->objReader      = \PhpOffice\PhpSpreadsheet\IOFactory::createReader( $inputFileType );
	...
	/**
	 * Creates \PhpOffice\PhpSpreadsheet\Spreadsheet object and template for report
	 */
	private function createTemplate() {
		...
		$this->objWorksheet->getPageSetup()->setOrientation( \PhpOffice\PhpSpreadsheet\Worksheet\PageSetup::ORIENTATION_PORTRAIT );
		$this->objWorksheet->getPageSetup()->setPaperSize( \PhpOffice\PhpSpreadsheet\Worksheet\PageSetup::PAPERSIZE_A4 );
	...
	public function createReport() {
		foreach ( $this->_data as $data ) {
			...
			if ( isset( $data['header'] ) ) {
				...
				if ( $colIndex > -1 ) {
					$this->objWorksheet->getStyle( \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex( 0 ) . $nextRow . ':' . \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex( $colIndex ) . $nextRow )->applyFromArray( $this->_headerStyleArray );
				} //$colIndex > -1
			...
			if ( isset( $data['footer'] ) ) {
				...
				if ( $colIndex > -1 ) {
					$this->objWorksheet->getStyle( \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex( 0 ) . $nextRow . ':' . \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex( $colIndex ) . $nextRow )->applyFromArray( $this->_footerStyleArray );
				} //$colIndex > -1
	...
	public function generateReport() {
		...
		foreach ( $this->_data as $data ) {
				...
				foreach ( $this->objWorksheet->getRowIterator() as $row ) {
					...
					foreach ( $cellIterator as $cell ) {
						...
						if ( preg_match_all( "/\{" . $data['id'] . ":(\w*|#\+?-?(\d*)?)\}/", $cellval, $matches ) ) {
							...
							if ( $firstRow == '' )
								$firstRow = $rowIndex;
						} //preg_match_all( "/\{" . $data['id'] . ":(\w*|#\+?-?(\d*)?)\}/", $cellval, $matches )
					} //$cellIterator as $cell
				} //$this->objWorksheet->getRowIterator() as $row
			...
			} //isset( $data['repeat'] ) && $data['repeat'] == true
			else {
				//non-repeating data
	...
	private function generateRepeatingRows( &$data, $repeatRange ) {
		...
		foreach ( $mergeCells as $mergeCell ) {
			if ( $this->isSubrange( $mergeCell, $repeatRange ) ) {
				...
				$needMerge[] = $mergeCell;
			} //$this->isSubrange( $mergeCell, $repeatRange )
		} //$mergeCells as $mergeCell
		...
		foreach ( $data['data'] as $value ) {
			...
			foreach ( $needMerge as $nm ) {
				$nm       = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::rangeBoundaries( $nm );
				$newMerge = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex( $nm[0][0] - 1 ) . ( $nm[0][1] + $skip ) . ":" . \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex( $nm[1][0] - 1 ) . ( $nm[1][1] + $skip );
	...
	private function generateRepeatingRowsWithGrouping( &$data, $repeatRange ) {
		...
		$firstRowFoundAt = reset( $templateKeys );
		$rowsFound       = count( $repeatTemplateArray );
		list( $rangeStart, $rangeEnd ) = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::rangeBoundaries( $repeatRange );
		$firstCol   = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex( $rangeStart[0] - 1 );
		$lastCol    = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex( $rangeEnd[0] - 1 );
		$mergeCells = $this->objWorksheet->getMergeCells();
		$needMerge  = array();
		foreach ( $mergeCells as $mergeCell ) {
			if ( $this->isSubrange( $mergeCell, $repeatRange ) ) {
		...
		foreach ( $this->_group['rows'] as $name => $rows ) {
			...
			} //isset( $this->_group['summary'] ) && isset( $this->_group['summary'][$name] )
			//remove merge on template, BUG fix
			foreach ( $needMerge as $nm ) {
				$this->objWorksheet->unmergeCells( $nm );
			} //$needMerge as $nm
		} //$this->_group['rows'] as $name => $rows
	}
	/**
	...
	private function generateSingleRepeatingRow( &$value, &$repeatTemplateArray, $rowCounter, $skip, $id, $format, $step ) {
		foreach ( $repeatTemplateArray as $rowKey => $rowData ) {
			foreach ( $rowData as $col => $tag ) {
					...
					foreach ( $matchKeys as $mkey ) {
						...
						elseif ( key_exists( $mkey, $value ) ) {
							...
							if ( isset( $format ) && isset( $format[$mkey] ) ) {
								...
								} //$format[$mkey] as $ftype => $f
							} //isset( $format ) && isset( $format[$mkey] )
							else {
								//without additional formating
								$rValue = $value[$mkey];
							}
						} //key_exists( $mkey, $value )
						else
							$rValue = $mkey;
				...
				} //preg_match_all( "/\{" . $id . ":(\w*|#\+?-?(\d*)?)\}/", $tag, $matches )
				...
				if ( $this->_useStripRows && $rowCounter % 2 ) {
					$this->objWorksheet->getStyle( $newCellAddress )->getFill()->setFillType( \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID );
					$this->objWorksheet->getStyle( $newCellAddress )->getFill()->getStartColor()->setRGB( 'F2F2F2' );
				} //$this->_useStripRows && $rowCounter % 2
			} //$rowData as $col => $tag
		} //$repeatTemplateArray as $rowKey => $rowData
	}
	/**
	...
	private function renderHtml() {
		$this->objWriter = new \PhpOffice\PhpSpreadsheet\Writer\Html( $this->objPHPExcel );
		// Generate HTML
		$html            = '';
	...
	private function renderXlsx( $filename ) {
		...
		$this->objWriter = \PhpOffice\PhpSpreadsheet\IOFactory::createWriter( $this->objPHPExcel, 'Xlsx' );
	...
	private function renderXls( $filename ) {
		...
		header( 'Cache-Control: max-age=0' );
		$this->objWriter = \PhpOffice\PhpSpreadsheet\IOFactory::createWriter( $this->objPHPExcel, 'Xls' );
		$this->objWriter->save( 'php://output' );
		unset( $this->objWriter );
		unset( $this->objWorksheet );
		unset( $this->objReader );
	...
	private function renderPdf( $filename ) {
		...
		header( 'Cache-Control: max-age=0' );
		$this->objWriter = \PhpOffice\PhpSpreadsheet\IOFactory::createWriter( $this->objPHPExcel, 'Pdf' );
		$this->objWriter->save( 'php://output' );
		exit();
	}
	/**
	...
	public function isSubrange( $subRange, $range ) {
		list( $rangeStart, $rangeEnd ) = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::rangeBoundaries( $range );
		list( $subrangeStart, $subrangeEnd ) = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::rangeBoundaries( $subRange );
	...
	/**
	 * Get \PhpOffice\PhpSpreadsheet\Spreadsheet object to add new parameters
	 */
	...
	private function renderCsv( $filename ) {
		...
		header( 'Cache-Control: max-age=0' );
		$this->objWriter = \PhpOffice\PhpSpreadsheet\IOFactory::createWriter( $this->objPHPExcel, 'CSV' );
		$this->objWriter->save( 'php://output' );
		unset( $this->objWriter );
		unset( $this->objWorksheet );
		unset( $this->objReader );
